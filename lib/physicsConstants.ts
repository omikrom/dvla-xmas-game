...
export const BASE_TURN_SPEED = 2.2; // reduced from 3.0 for wider turning circle
...
export const CLIENT_REPLAY_SUBSTEPS = Number(
  process.env.CLIENT_REPLAY_SUBSTEPS || 6
);
// Distance (units) above which corrections are treated as "big" — duration will scale up instead of snapping.
export const CLIENT_BIG_CORRECTION_DIST = Number(
  process.env.CLIENT_BIG_CORRECTION_DIST || 1.5
);
// Maximum smoothing time for large corrections (ms)
export const CLIENT_BIG_CORRECTION_MAX_MS = Number(
  process.env.CLIENT_BIG_CORRECTION_MAX_MS || 300
);
